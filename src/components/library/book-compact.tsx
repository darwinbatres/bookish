...
export function BookCompact({
  ...
  const loadCollections = useCallback(() => {
    fetchCollections()
      .then(setCollections)
      .catch((err) =>
        console.error("[BookCompact] Failed to load collections:", err)
      );
  }, []);
  ...
  const handleMoveToCollection = useCallback(
    async (bookId: string, collectionId: string | null, bookTitle: string) => {
      try {
        ...
        console.error("[BookCompact] Failed to move book:", err);
  ...
  const handleToggleFavorite = useCallback(
    async (book: DBBook) => {
      try {
        ...
        console.error("[BookCompact] Failed to toggle favorite:", err);
  ...
  const handleDownloadBook = useCallback(async (book: DBBook) => {
    try {
      ...
      console.error("[BookCompact] Failed to download book:", err);
  ...
  const handleDeleteBook = useCallback(async () => {
    ...
    try {
      ...
      console.error("[BookCompact] Failed to delete book:", err);
                ...
                <DropdownMenu>
                  ...
                  >
                    <Button
                      size="sm"
                      variant="ghost"
                      className="h-7 w-7 p-0 opacity-0 group-hover:opacity-100 transition-all"
                    >
