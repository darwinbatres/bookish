/**
 * Media Folder Cover Upload API
 * POST /api/media-folders/cover-upload - Upload a folder cover image
...
async function handler(
  ...
  try {
    ...
    // Get folderId from fields
    const folderIdField = fields.folderId;
    const folderId = Array.isArray(folderIdField)
      ? folderIdField[0]
      : folderIdField;
    ...
    const s3Key = generateMediaFolderCoverS3Key(folderId, originalFilename);
    ...
    console.error("[API] Media folder cover upload error:", error);
