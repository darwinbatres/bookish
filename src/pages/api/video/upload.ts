/**
 * Video Upload API
 * POST /api/video/upload - Upload a video file (proxied to S3)
...
import { getVideoMaxSizeMB } from "@/lib/db";
...
async function handler(
  ...
  try {
    // Get max file size from database settings for video
    const videoMaxSizeMB = await getVideoMaxSizeMB();
    ...
    if (!isValidVideoContentType(contentType)) {
      ...
      return res.status(400).json({
        error: "Bad Request",
        message: `Invalid file type: ${contentType}. Allowed: mp4, webm, mkv, mov, avi, m4v`,
        statusCode: 400,
      });
    }
    ...
    // Generate S3 key
    const originalFilename = file.originalFilename || "video.mp4";
    const s3Key = generateVideoS3Key(videoId, originalFilename);
