...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  req.on("close", () => {
    if (streamStarted) {
      console.log("[API] Client disconnected, cleaning up video stream");
      cleanup();
    }
  });
  ...
  try {
    ...
    if (!response.Body) {
      return res.status(500).json({
        ...
        message: "Failed to stream video file - no body returned",
        statusCode: 500,
      });
    }
    ...
    const resetIdleTimer = () => {
      ...
      idleTimer = setTimeout(() => {
        console.error("[API] Video stream idle timeout for:", s3Key);
        cleanup();
    ...
    stream.on("error", (error) => {
      ...
      if (!res.headersSent) {
        res.status(500).json({
          error: "Internal Server Error",
          message: "Stream error while reading video file",
          statusCode: 500,
        });
